...
- **開発期間**: 2025.10.24 ~ 2025.11.09（2週間）
- **参加人数**：6名
...
### 💡 主な担当業務と成果
* **REST API およびサーバーレスバックエンドの構築**
  * AWS Lambda を用いてバックエンドロジックを実装し、Amazon API Gateway と統合して REST API を構築しました。
* **S3 署名付きURLを活用した安全なファイルアップロード機能の実装**
  * S3 Pre-signed URL を動的に生成してクライアントへ配信し、安全かつ効率的なファイルアップロード機能を実装しました。
* **イベント駆動型アーキテクチャの設計**
  * Amazon SQS を活用して後続処理を非同期化し、イベント駆動型のバックエンド構成を設計しました。
---
